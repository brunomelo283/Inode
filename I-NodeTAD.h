...
int addDiretorioEArquivo(Disco disco[], char tipoArquivo, int enderecoInodeDiretorioAtual, char nomeDiretorioArquivoNovo[MAX_NOME_ARQUIVO], int enderecoInodeRaiz=-1, int tamanhoArquivo = 1, string caminhoLink = "", int enderecoInodeCriado = getEnderecoNull());
int addDiretorioEArquivo(Disco disco[], char tipoArquivo, int enderecoInodeDiretorioAtual, char nomeDiretorioArquivoNovo[MAX_NOME_ARQUIVO], int enderecoInodeRaiz, int tamanhoArquivo = 1, string caminhoLink = "", int enderecoInodeCriado = getEnderecoNull());
...
int addDiretorioEArquivo(Disco disco[], char tipoArquivo, int enderecoInodeDiretorioAtual, char nomeDiretorioArquivoNovo[MAX_NOME_ARQUIVO], int enderecoInodeRaiz, int tamanhoArquivo = 1, string caminhoLink = "", int enderecoInodeCriado = getEnderecoNull())
{
    ...
    if(nomeDiretorioArquivoNovo[0]  == '/')
    {
        caminhoOrigem.push_back("/");

        for (const string &elem : split(nomeDiretorioArquivoNovo, '/'))
        {
    ...
    {
        caminhoOrigem = split(nomeDiretorioArquivoNovo, '/');
    }

    if(caminhoOrigem.size() > 1)
    {
        for (const auto &str : caminhoOrigem)
        {
                enderecoInodeArquivo = cd(disco, enderecoInodeArquivo, str.c_str(), enderecoInodeRaiz, stringAux);
...
void linkSimbolico(Disco disco[], int enderecoInodeAtual, string comando, int enderecoInodeRaiz)
{
    ...
    if (caminhosOrigemDestino.size() == 2)
    {
        ...
        addDiretorioEArquivo(disco, TIPO_ARQUIVO_LINK, enderecoInodeAtual, caminhoDestinoChar, -1, 1, caminhoAux);
    }
}
    ...
    if (caminhosOrigemDestino.size() == 2)
    {
        ...
        if (isEnderecoValido(enderecoInodeOrigem))
        {
            ...
            addDiretorioEArquivo(disco, disco[enderecoInodeOrigem].inode.protecao[0],
                                 enderecoInodeDestino, caminhoDestinoChar,-1, 1, caminhoAux,
                                 enderecoInodeOrigem);
        }
    }
}
